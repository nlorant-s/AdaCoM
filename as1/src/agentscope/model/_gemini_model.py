...
class GeminiChatModel(ChatModelBase):
    ...
    Supports two modes:
    - Native mode: Uses google-genai SDK to call Google's Gemini API directly.
    - Proxy mode: When ``client_args`` contains ``base_url``, uses raw HTTP
      to send Gemini-native format requests through an OpenAI-compatible
      proxy (e.g. DashScope). This is needed because the google-genai SDK
      constructs URL paths incompatible with proxy endpoints.
    """
    ...
    def __init__(
        ...
        Args:
            model_name (`str`):
                ...
                or "vertex_ai.gemini-2.5-flash" (for proxy mode).
            api_key (`str`):
                The API key for Google Gemini or the proxy service.
            ...
            client_args (`dict`, default `None`):
                Extra keyword arguments. If ``base_url`` key is present,
                enables proxy mode (raw HTTP via httpx). Otherwise passed
        ...
        self._proxy_base_url = client_args.pop("base_url", None)
        self._api_key = api_key
        self.thinking_config = thinking_config
        self.generate_kwargs = generate_kwargs or {}

        if self._proxy_base_url:
            # Proxy mode: use raw HTTP (httpx)
            self.client = None
            logger.info(
                "[GeminiChatModel] Proxy mode: model=%s, base_url=%s",
                model_name,
                self._proxy_base_url,
            )
        else:
            # Native mode: use google-genai SDK
            try:
    ...
    async def __call__(
        ...
        **config_kwargs: Any,
        ...
        if self._proxy_base_url:
            # Proxy mode: raw HTTP with Gemini native format
            parsed_response = await self._call_proxy(
                messages, config, structured_model, start_datetime,
            )
            return self._call_post_llm_hooks(
                parsed_response, *original_args, **original_kwargs,
            )
        ...
        if self.stream:
            ...
            return self._parse_gemini_stream_generation_response_with_hooks(
                start_datetime,
                response,
                structured_model,
                original_args,
    ...
    async def _call_proxy(
        ...
        """Call Gemini model through a proxy endpoint using raw HTTP.

        Sends Gemini-native format to the proxy's /chat/completions endpoint
        and parses the Gemini-native response.
        """
        import httpx
        ...
        # Map config fields to Gemini REST API format
        # Skip fields not recognized by Gemini REST API
        _skip_keys = {"stream"}
        ...
        for k, v in config.items():
            ...
            if k == "thinking_config":
                # DashScope's vertex_ai.* channel expects Gemini-native
                # generationConfig.thinkingConfig with camelCase keys.
                tc: dict[str, Any] = {}
                if "include_thoughts" in v:
                    tc["includeThoughts"] = v["include_thoughts"]
                if "thinking_budget" in v:
                    tc["thinkingBudget"] = v["thinking_budget"]
                if tc:
                ...
                body["tool_config"] = v
            elif k == "response_mime_type":
                gen_config["responseMimeType"] = v
            elif k == "response_schema":
                gen_config["responseSchema"] = v
            else:
                gen_config[k] = v
        ...
        url = f"{self._proxy_base_url.rstrip('/')}/chat/completions"
        async with httpx.AsyncClient(timeout=300) as http:
            resp = await http.post(url, json=body, headers=headers)
            if resp.status_code >= 400:
                raise RuntimeError(
                    f"Gemini proxy error {resp.status_code}: "
                    f"{resp.text[:500]}",
                )
            data = resp.json()

        return self._parse_proxy_response(data, start_datetime, structured_model)
    ...
    def _parse_proxy_response(
        ...
        """Parse Gemini-native JSON response from the proxy."""
            ...
            for part in candidate.get("content", {}).get("parts", []):
                if part.get("thought") and part.get("text"):
                    content_blocks.append(
                        ThinkingBlock(type="thinking", thinking=part["text"]),
                    )
                elif "text" in part:
                    text += part["text"]
    ...
    async def _parse_gemini_stream_generation_response_with_hooks(
        self,
        start_datetime: datetime,
        response: AsyncIterator[GenerateContentResponse],
        structured_model: Type[BaseModel] | None = None,
        ...
        """Wrap stream parsing with post-LLM hooks."""
        async for chat_response in self._parse_gemini_stream_generation_response(
            start_datetime, response, structured_model,
        ):
            final_response = self._call_post_llm_hooks(
                chat_response, *original_args, **(original_kwargs or {}),
            )
