...
class OpenAIChatModel(ChatModelBase):
    ...
    async def __call__(
        ...
        **kwargs: Any,
        ...
        hook_kwargs = self._call_pre_llm_hooks(*original_args, **original_kwargs)
        
        # Extract potentially modified parameters
        messages = hook_kwargs.get('args', original_args)[0]
        tools = hook_kwargs.get('tools', tools)
        tool_choice = hook_kwargs.get('tool_choice', tool_choice)
        structured_model = hook_kwargs.get('structured_model', structured_model)
        kwargs = {k: v for k, v in hook_kwargs.items() 
                 if k not in ['args', 'tools', 'tool_choice', 'structured_model']}
        ...
        if structured_model:
            ...
            if not self.stream:
                ...
                # For streaming structured responses, wrap with hooks
                async def _wrapped_structured_stream():
                    async for chat_response in self._parse_openai_stream_response(
                        start_datetime,
                        response,
                        structured_model,
                    ):
                ...
                return _wrapped_structured_stream()
        else:
            response = await self.client.chat.completions.create(**kwargs)

        if self.stream:
            # For streaming responses, we need to wrap the generator to call post hooks
