...
class ReActAgent(ReActAgentBase):
    ...
    def __init__(
        ...
        Args:
            ...
            experiment_logger (`Any`, optional):
    ...
    async def _reasoning(
        ...
        # Log LLM invocation if experiment logger is available
        if self.experiment_logger:
            # Convert prompt to messages format for logging
            messages_for_log = []
            if hasattr(prompt, '__iter__'):
                for msg in prompt:
                    if isinstance(msg, dict):
                        ...
                        messages_for_log.append({"role": "user", "content": str(msg)})
            else:
                messages_for_log.append({"role": "user", "content": str(prompt)})
        ...
        try:
            ...
            if interrupted_by_user and msg:
                ...
                for tool_call in tool_use_blocks:
                    ...
                    # Commented out to suppress tool result output to console
            ...
            if self.enable_rewrite_query:
                try:
                    ...
                    if self.experiment_logger:
                        # Convert prompt to messages format for logging
                        messages_for_log = []
                        if hasattr(rewrite_prompt, '__iter__'):
                            for msg in rewrite_prompt:
                                if isinstance(msg, dict):
                                    messages_for_log.append(msg)
                                else:
                                    messages_for_log.append({"role": "user", "content": str(msg)})
                        else:
                            messages_for_log.append({"role": "user", "content": str(rewrite_prompt)})
