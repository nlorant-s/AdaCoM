...
class Msg:
    ...
    def get_content_blocks(
        ...
        if isinstance(self.content, str):
            ...
            # Coerce stray str items into text blocks. Memory modifications can
            # land mixed list content here and formatters assume dict blocks.
            for b in (self.content or []):
                if isinstance(b, str):
                    blocks.append(TextBlock(type="text", text=b))
                elif isinstance(b, dict):
                    blocks.append(b)

        if block_type is not None:
            blocks = [_ for _ in blocks if _.get("type") == block_type]
