    ...
    def train_step(self, batch: Experiences) -> Dict:  # noqa C901
        ...
        with marked_timer("step", timing_raw):
            ...
            if self.algorithm.can_balance_batch and self.config.trainer.balance_batch:
                # Trim batch to be divisible by world_size (required by karmarkar_karp equal_size=True)
                world_size = self.actor_rollout_wg.world_size
                batch_size = batch.batch["attention_mask"].shape[0]
                remainder = batch_size % world_size
                if remainder != 0:
                    trimmed_size = batch_size - remainder
                    self.logger.warning(
                        f"Trimming batch from {batch_size} to {trimmed_size} "
                        f"(dropping {remainder} samples) for balanced partitioning across {world_size} GPUs"
                    )
                    batch = batch[:trimmed_size]
