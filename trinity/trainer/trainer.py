...
class Trainer:
    ...
    def __init__(self, config: Config) -> None:
        ...
        self.save_satisfied_interval = config.trainer.save_satisfied_interval or config.trainer.save_interval
        ...
        # Track whether metric is above threshold (used for dynamic save logic)
        self.metric_above_threshold = False
        # Track last saved step to avoid duplicate saves
    ...
    async def train_step(self, exps: Experiences) -> Dict:
        ...
        try:
            ...
            self.logger.error(f"CRITICAL ERROR INSIDE train_step (step {self.train_step_num + 1}): {e}")
    ...
    def need_save(self) -> bool:
        ...
        - Before threshold is first met: save by save_interval
        - After threshold is first met (latched): save by save_satisfied_interval
        """
        ...
        if self.config.trainer.save_on_metric:
            if self.metric_above_threshold:
                return self.save_satisfied_interval > 0 and self.train_step_num % self.save_satisfied_interval == 0
            return self.save_interval > 0 and self.train_step_num % self.save_interval == 0
        return self.save_interval > 0 and self.train_step_num % self.save_interval == 0

    async def set_metric_above_threshold(self, above_threshold: bool) -> None:
        """Set whether metric is above threshold (called by explorer after eval)."""
        old_state = self.metric_above_threshold
        self.metric_above_threshold = above_threshold
        if old_state != self.metric_above_threshold:
            self.logger.info(
                f"Trainer metric_above_threshold changed: {old_state} -> {self.metric_above_threshold}"
            )

    def should_save_by_interval(self) -> bool:
        """Check if current step should save by interval (for explorer query)."""
        return self.save_interval > 0 and self.train_step_num % self.save_interval == 0
    ...
    def save_checkpoint(self, block_until_saved: bool = False, save_as_hf: bool = False) -> Dict:
        ...
        with Timer(metrics, "time/save_checkpoint"):
            ...
            # Update last_save_step to avoid duplicate saves
    ...
    async def save_checkpoint_on_metric(self) -> None:
        """Save checkpoint triggered by eval metric threshold."""
        self.logger.info(f"Metric-based checkpoint save at step {self.train_step_num}")
        self.save_checkpoint(save_as_hf=self.save_hf_checkpoint == "always")
