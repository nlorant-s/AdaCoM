...
def copy_config_to_checkpoint_dir(config_path: str, config: Config) -> None:
    """Copy the YAML config file to the checkpoint directory."""
    if not os.path.exists(config_path):
        logger.warning(f"Config file {config_path} does not exist, skipping copy.")
        return
    
    try:
        # Ensure checkpoint_job_dir exists (it should be created in config.check_and_update())
        os.makedirs(config.checkpoint_job_dir, exist_ok=True)
        
        # Get the original config filename
        config_filename = os.path.basename(config_path)
        
        # Copy the config file to checkpoint_job_dir
        dest_path = os.path.join(config.checkpoint_job_dir, config_filename)
        shutil.copy2(config_path, dest_path)
        
        logger.info(f"Copied config file from {config_path} to {dest_path}")
        ...
        if config.stages:
            ...
            for i, stage_config in enumerate(config):
                if i < latest_stage:
                    ...
                    copy_config_to_checkpoint_dir(config_path, stage_config)
            ...
            copy_config_to_checkpoint_dir(config_path, config)
            run_stage(config)
