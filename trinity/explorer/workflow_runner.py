...
def calculate_run_level_metrics(experiences: List[Experience]) -> Dict[str, float]:
    ...
    """
    run_level_metrics: Dict[str, list] = defaultdict(list)
    ...
    for key, values in run_level_metrics.items():
        ...
        elif isinstance(values[0], str) and len(set(values)) == 1:
            # Preserve string values if consistent across all experiences (e.g. _agent_model)
    ...
    async def _run_task(
        ...
        if self.workflow_instance.repeatable:
            ...
            for i in range(repeat_times):
                ...
                # Pass current run_id to workflow so it can differentiate logging per run
                if self.workflow_instance is not None:
                    self.workflow_instance.current_run_id = run_id_base + i
                self.runner_state["terminate_time"] = None
                self.runner_state["begin_time"] = st
