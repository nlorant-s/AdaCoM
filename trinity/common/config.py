...
class FormatConfig:
    ...
    task_id_key: str = "task_id"  # Field name for task identifier in the dataset
...
class OverRolloutConfig:
    ...
    eval_ratio: float = -1.0  # over_rollout ratio for eval; -1 means inherit from `ratio`, 0 means wait for all
...
class TasksetConfig:
    ...
    repeat_times: int = 1
    # Per-taskset eval interval (optional). If set, this taskset only runs when
    # explore_step_num % eval_interval == 0. Falls back to global eval_interval if unset (0).
    eval_interval: int = 0
    # Max number of tasks to evaluate per eval run (0 = all). If set, tasks are randomly sampled.
...
class ExplorerConfig:
    ...
    # Dynamic eval interval based on metric threshold
    # When metric < threshold: use eval_interval_low_score and eval_repeat_times_low_score
    # When metric >= threshold: use eval_interval_high_score and eval_repeat_times_high_score
    # Set to None to disable dynamic eval interval (use fixed eval_interval)
    eval_interval_low_score: Optional[int] = None
    eval_interval_high_score: Optional[int] = None
    eval_repeat_times_low_score: int = 1
    eval_repeat_times_high_score: int = 3
...
class TrainerConfig:
    ...
    # Metric-based checkpoint saving
    save_on_metric: Optional[str] = None  # metric name, e.g. "eval/math-eval/accuracy/mean"
    save_on_metric_threshold: float = 0.0  # save when metric > threshold
    save_satisfied_interval: Optional[int] = None  # save interval after metric threshold is met (defaults to save_interval)
    save_only_on_passing_eval: bool = False  # only save checkpoints after eval when metric > threshold
...
class Config:
    ...
    # User-defined variables for configuration reuse
    dataset: Optional[str] = None  # Dataset name for variable substitution
    mem_config: Optional[str] = None  # Memory config preset name for variable substitution
    ...
    def _check_interval(self) -> None:
        ...
        if self.mode != "bench" and self.algorithm.algorithm_type != "dpo":  # TODO
            ...
            # check dynamic eval interval configuration
            if self.explorer.eval_interval_low_score is not None:
                # Validate that save_on_metric is configured (required for dynamic eval)
                if not self.trainer.save_on_metric:
                    raise ValueError(
                        "Dynamic eval interval requires `trainer.save_on_metric` to be configured. "
                        "Please set `trainer.save_on_metric` to the metric name to track."
                    )

                # Validate eval_interval_high_score is also set
                if self.explorer.eval_interval_high_score is None:
                    raise ValueError(
                        "`explorer.eval_interval_high_score` must be set when `explorer.eval_interval_low_score` is configured."
                    )

                # Adjust eval_interval_low_score to be a multiple of sync_interval
                if self.explorer.eval_interval_low_score % self.synchronizer.sync_interval != 0:
                    self.explorer.eval_interval_low_score = (
                        max(self.explorer.eval_interval_low_score // self.synchronizer.sync_interval, 1)
                    ) * self.synchronizer.sync_interval
                    logger.warning(
                        f"`eval_interval_low_score` is not a multiple of `sync_interval`; adjusted to the nearest integer={self.explorer.eval_interval_low_score}."
                    )

                # Adjust eval_interval_high_score to be a multiple of sync_interval
                if self.explorer.eval_interval_high_score % self.synchronizer.sync_interval != 0:
                    self.explorer.eval_interval_high_score = (
                        max(self.explorer.eval_interval_high_score // self.synchronizer.sync_interval, 1)
                    ) * self.synchronizer.sync_interval
                    logger.warning(
                        f"`eval_interval_high_score` is not a multiple of `sync_interval`; adjusted to the nearest integer={self.explorer.eval_interval_high_score}."
                    )
    ...
    def check_and_update(self) -> Config:  # noqa: C901
        ...
        # create a job dir at checkpoint_root_dir/project/dataset/group/name
        self.checkpoint_job_dir = os.path.join(
            self.checkpoint_root_dir, self.project, self.dataset or "default", self.group, self.name
        )
