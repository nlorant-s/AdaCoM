...
class QueueStorage:
    ...
    async def put_back(self, exp_list: List) -> None:
        """Put experiences back to the front of exp_pool (no file write)."""
        self.exp_pool.extendleft(reversed(exp_list))

    async def get_batch(
        self, batch_size: int, timeout: float, min_version: Optional[int] = None
    ) -> List:
        """Get batch of experience.

        If ``min_version`` is set, experiences with ``model_version < min_version`` are
        dropped at dequeue time (never enter ``exp_pool``). This lets the outer while
        loop keep awaiting fresh data via ``queue.get()`` (asyncio.Condition.wait) instead
        of busy-looping at the caller side.
        """
        ...
        while len(self.exp_pool) < batch_size:
            ...
            try:
                ...
                if min_version is not None:
                    fresh = [
                        e for e in exp_list if e.info.get("model_version", 0) >= min_version
                    ]
                    dropped_stale += len(exp_list) - len(fresh)
                    self.exp_pool.extend(fresh)
                else:
                ...
                if time.time() - start_time > timeout:
                    self.logger.error(
                        ...
                        f"(dropped {dropped_stale} stale with min_version={min_version}).\n"
        ...
        if dropped_stale > 0:
            self.logger.info(
                f"Dropped {dropped_stale} stale experiences (min_version={min_version})."
            )
